...
from models.harrisNet import ImageGradientsLayer
...
class HistogramLayer(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        ** You will receive extra-credit if you successfully implement this
        ...
        # max indices per-pixel across the 8 cosine channels -> shape (1, m, n)
        max_idx = torch.argmax(cosines, dim=1).squeeze(0).flatten().long()
        _, _, m, n = cosines.shape
        # create grid of pixel coordinates (height indices, width indices)
        grid_x, grid_y = torch.meshgrid(
            torch.arange(m, device=cosines.device),
            torch.arange(n, device=cosines.device),
            indexing="ij",
        )
        m_idx = grid_x.flatten().long()
        n_idx = grid_y.flatten().long()
        per_px_histogram[0, max_idx, m_idx, n_idx] = 1.0
        magnitudes = torch.norm(im_grads, dim=1, keepdim=True)
        per_px_histogram = per_px_histogram * magnitudes
...
class SIFTOrientationLayer(nn.Module):
    ...
    def __init__(self):
        ...
        layer = nn.Conv2d(in_channels=2, out_channels=10, kernel_size=1, bias=False)
    ...
    def get_orientation_bin_weights(self) -> torch.nn.Parameter:
        ...
        orientation_radians = ratios * torch.pi
        orientation_vectors = (
            angles_to_vectors_2d_pytorch(orientation_radians)
            .unsqueeze(-1)
            .unsqueeze(-1)
        )
        unit = (
            torch.tensor([[1.0, 0.0], [0.0, 1.0]], dtype=orientation_vectors.dtype)
            .unsqueeze(-1)
            .unsqueeze(-1)
        )
        weight_param = nn.Parameter(torch.cat((orientation_vectors, unit), dim=0))
        return weight_param
...
class SIFTNet(nn.Module):
    ...
    def __init__(self):
        ...
        model = torch.nn.Sequential(igl, sol, hl, sgal)
        self.net = model
