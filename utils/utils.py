...
def show_correspondence_circles(imgA, imgB, X1, Y1, X2, Y2):
    ...
    for x1, y1, x2, y2 in zip(X1, Y1, X2, Y2):
        ...
        draw.ellipse(
            [x2 + shiftX - r + 1, y2 - r + 1, x2 + shiftX + r - 1, y2 + r - 1],
            fill=cur_color,
            outline=green,
        )
...
def show_correspondence_lines(imgA, imgB, X1, Y1, X2, Y2, line_colors=None):
    ...
    for x1, y1, x2, y2, dot_color, line_color in zip(
        ...
        draw.ellipse(
            (x2 + shiftX - r, y2 - r, x2 + shiftX + r, y2 + r), fill=tuple(dot_color)
        )
...
def evaluate_correspondence(
    ...
    for i in priority:
        ...
        gt_offset = np.asarray(
            [x1[match_idx] - x2[match_idx], y1[match_idx] - y2[match_idx]]
        )
    ...
    accuracy = (
        np.mean(good_matches)
        * min(num_matches, num_req_matches)
        * 1.0
        / num_req_matches
    )
    ...
    return accuracy, show_correspondence_lines(
        imgA,
        imgB,
        x1_est * scale_factor,
        y1_est * scale_factor,
