...
from src.data_transforms import get_fundamental_transforms
...
def test_dataset_length():
    try:
        train_image_loader = ImageLoader(
            root_dir="data/",
            split="train",
            transform=get_fundamental_transforms(
                inp_size=(64, 64),
                pixel_mean=np.array([0.01]),
                pixel_std=np.array([1.001]),
            ),
        )
...
def test_load_img_from_path():
    ...
    try:
        expected_data = np.loadtxt("proj4_unit_tests/data/sample_inp.txt")
    except:
        expected_data = np.loadtxt("../proj4_unit_tests/data/sample_inp.txt")
